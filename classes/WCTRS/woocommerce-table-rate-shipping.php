...
function Printaura_woocommerce_table_rate_shipping_init_pa() {
	...
	if ( class_exists( 'Woocommerce' ) || class_exists( 'WooCommerce' ) ) {
		   $table_rate_shipping_enabled = get_option('printaura_woocommerce_api_table_rate_shipping');
			...
			class Printaura_BE_Table_Rate_Shipping extends WC_Shipping_Method {
				...
				*/
				...
				function calculate_shipping( $package = array() ) {
					...
					if($this->ship_free != '') {
						if( $cart_subtotal >= (float) apply_filters( 'wcml_raw_price_amount', $this->ship_free ) ) {
			    	...
			    	if(count($rate) > 0) {
						...
						if(count($this->title_order)) {
							...
							foreach( $title_order as $key => $title )
								$title_order[ $key ] = $this->id . '_' . $title;

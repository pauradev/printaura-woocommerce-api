...
if ( is_array( $images ) ) {
			...
			foreach ( $images as $image ) {
				...
				if ( isset( $image['position'] ) && 0 == $image['position'] ) {
					...
					else set_post_thumbnail( $product_id, $attachment_id );
...
$images_ids[] = $attachment_id;
			...
			if ( ! empty( $gallery ) ) {
				...
				} else update_post_meta( $product_id, '_product_image_gallery', implode( ',', $gallery ) );
			}
		} else {
			if(method_exists($product,'set_gallery_image_ids')){
...
function printaura_pa_get_images( $product ) {
		...
		} else $attachment_ids = array_merge( $attachment_ids, $product->get_gallery_attachment_ids() );
		// Build image data.
